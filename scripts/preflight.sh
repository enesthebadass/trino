...
check "Java 24 available on host" "java -version 2>&1 | grep -q 'version \"24'"
check "Python 3.9 or newer" "python3 - <<'PY'
import sys
...
if [ "$failures" -gt 0 ]; then
  printf '\n%d preflight check(s) failed. Fix host settings before production use.\n' "$failures"
  exit 1
fi
